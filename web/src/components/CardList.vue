<template>
  <div class="row">
    ...
    >
      <div class="content">
        <a :title="item.title" :href="item.source">
          <figure>
            <img class="lazyload" :src="item.img" :alt="item.title" />
            <div class="p-info"></div>
          </figure>
        </a>
        <a :title="item.title" :href="item.source" class="item-name">
        ...
        <hr />
        <a :href="item.authorSource" class="who">{{ item.author }}</a> <!-- who alternate gelebilir -->
        ...
        <span class="price"
          >{{ (item.price - item.price * (item.discount / 100)).toFixed(2) }} TL</span
        >
      </div>
    </div>
  </div>
